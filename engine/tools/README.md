...
- `testbuildinfo.sh`: Builds and validates the packaged engine, starts a local node, and requires the real Go Engine Build Information collector canary to pass. Run it through `make test-engine-build-info` from the repository root; use `--skip-build` only when intentionally validating an existing packaged JAR.
