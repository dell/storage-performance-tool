...
class WeightedRangeFactoryTest {
	@Test
	void mergedContextPolicyReachesRangeFactory() throws Exception {
		...
		final var schema = TreeUtil.reduceForest(List.of(baseSchema, new WeightedLoadStepExtension().schemaProvider().schema()));
		...
		baseConfig.val("load-op-weight", 1);
		...
		final var step = new WeightedLoadStepLocal(baseConfig, List.of(factory), List.of(override), mock(MetricsManager.class));
		try {
