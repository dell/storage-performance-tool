...
import static com.dell.spt.base.item.op.OpType.CREATE;
import static com.dell.spt.storage.driver.coop.netty.http.DellConstants.KEY_X_Dell_FILESYSTEM_ACCESS_ENABLED;
import static com.dell.spt.storage.driver.coop.netty.http.DellConstants.KEY_X_Dell_NAMESPACE;
import static com.dell.spt.storage.driver.coop.netty.http.DellConstants.KEY_X_Dell_SIGNATURE;
import static com.dell.spt.storage.driver.coop.netty.http.DellConstants.KEY_X_Dell_UID;
import static com.dell.spt.storage.driver.coop.netty.http.DellConstants.PREFIX_KEY_X_Dell;
import static com.dell.spt.storage.driver.coop.netty.http.atmos.AtmosApi.HEADERS_CANONICAL;
import static com.dell.spt.storage.driver.coop.netty.http.atmos.AtmosApi.KEY_SUBTENANT_ID;
...
import static com.dell.spt.storage.driver.coop.netty.http.atmos.AtmosApi.SIGN_METHOD;
import static com.dell.spt.storage.driver.coop.netty.http.atmos.AtmosApi.SUBTENANT_URI_BASE;
...
public class AtmosStorageDriver<I extends Item, O extends Operation<I>>
	...
	public AtmosStorageDriver(
		...
		if (namespace != null && !namespace.isEmpty()) {
			sharedHeaders.set(KEY_X_Dell_NAMESPACE, namespace);
		}
	...
	protected final String requestNewAuthToken(final Credential credential) {
		...
		// reqHeaders.set(KEY_X_Dell_DATE, reqHeaders.get(HttpHeaderNames.DATE));
		if (fsAccess) {
			reqHeaders.set(KEY_X_Dell_FILESYSTEM_ACCESS_ENABLED, Boolean.toString(fsAccess));
		}
	...
	protected final void applyAuthHeaders(
		...
		if (uid != null && !uid.isEmpty()) {
			if (authToken != null && !authToken.isEmpty() && !dstUriPath.equals(SUBTENANT_URI_BASE)) {
				httpHeaders.set(KEY_X_Dell_UID, authToken + '/' + uid);
			} else {
				httpHeaders.set(KEY_X_Dell_UID, uid);
			}
		}
		...
		if (secret != null && !secret.isEmpty()) {
			...
			httpHeaders.set(KEY_X_Dell_SIGNATURE, BASE64_ENCODER.encodeToString(sigData));
		}
	}
