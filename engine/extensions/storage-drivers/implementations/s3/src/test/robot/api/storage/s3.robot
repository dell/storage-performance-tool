*** Settings ***
Documentation  Spt S3 Storage Driver Tests
Force Tags  S3
Library  Collections
...
${SPT_SHARED_ARGS} =  --storage-driver-type=s3 --storage-net-node-port=${S3_PORT} --storage-auth-uid=${S3_UID} --storage-auth-secret=${S3_SECRET_KEY}
...
Should Read Single Random Byte Ranges
    ...
    ${std_out} =  Execute Spt Scenario  ${DATA_DIR}  ${env_params}  ${SPT_SHARED_ARGS} ${args}
    Log  ${std_out}
