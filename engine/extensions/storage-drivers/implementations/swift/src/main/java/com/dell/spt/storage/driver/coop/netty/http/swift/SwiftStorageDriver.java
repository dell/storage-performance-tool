...
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.AUTH_URI;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.DEFAULT_VERSIONS_LOCATION;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.KEY_X_AUTH_KEY;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.KEY_X_AUTH_TOKEN;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.KEY_X_AUTH_USER;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.KEY_X_COPY_FROM;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.KEY_X_OBJECT_MANIFEST;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.KEY_X_VERSIONS_LOCATION;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.MAX_LIST_LIMIT;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.URI_BASE;
import static com.dell.spt.storage.driver.coop.netty.http.swift.SwiftApi.parseContainerListing;
		...
		for (var i = from; i < to; i++) {
			...
			if (nextOp instanceof CompositeDataOperation) {
				...
				if (compositeOp.allSubOperationsDone()) {
					...
					final var n = subOps.size();
					if (n > 0) {
						// submit() blocks on concurrencyThrottle.acquire() if saturated
						for (var j = 0; j < n; j++) {
							while (!super.submit(subOps.get(j))) {
								LockSupport.parkNanos(1);
							}
						}
					} else {
