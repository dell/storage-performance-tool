*** Settings ***
Documentation  Spt Swift Storage Driver Tests
Force Tags  Swift
Library  Collections
Library  String
...
${SPT_SHARED_ARGS} =  --storage-driver-type=swift --storage-namespace=AUTH_test --storage-net-node-port=${SWIFT_PORT} --storage-auth-uid=${SWIFT_UID}
...
Should Read Multiple Random Byte Ranges
    ...
    ...  --item-data-verify
    ...  --run-scenario=${SPT_CONTAINER_DATA_DIR}/${step_id}.js
    ...  --storage-driver-limit-concurrency=10
    &{env_params} =  Create Dictionary  ITEM_LIST_FILE=${SPT_CONTAINER_DATA_DIR}/${step_id}.csv  RANDOM_BYTE_RANGE_COUNT=${random_byte_range_count}
    ${std_out} =  Execute Spt Scenario  ${DATA_DIR}  ${env_params}  ${SPT_SHARED_ARGS} ${args} --storage-auth-secret=${SWIFT_SECRET_KEY}
    Log  ${std_out}
...
Should Copy Objects Using Container Listing
    ...
    ...  --load-step-id=${step_id}
    ...  --storage-driver-limit-concurrency=10
    ...  --run-scenario=${SPT_CONTAINER_DATA_DIR}/copy_using_input_path.js
...
Should Create Dynamic Large Objects
    ...
    ...  --item-output-file=${SPT_CONTAINER_DATA_DIR}/${step_id}.csv
