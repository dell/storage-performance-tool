...
import com.dell.spt.base.item.io.{CsvFileItemInput, CsvFileItemOutput, NewDataItemInput}
...
final class CsvFileItemInputPerfTest {
	...
	def testInputRate(): Unit = {
		...
		Assert.assertEquals(count.toFloat, inputCounter.sum.toFloat, BATCH_SIZE.toFloat)
