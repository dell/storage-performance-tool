...
public class TestConfigBuilder {
	...
	/**
	 * Create minimal schema as fallback if resource loading fails.
	 * This covers the essential paths used by LoadStepClientBase.
	 */
	private static Map<String, Object> createMinimalSchema() {
		...
		Map<String, Object> stepLimit = new HashMap<>();
		stepLimit.put("size", Object.class);
		stepLimit.put("time", Object.class);
		step.put("limit", stepLimit);
		...
		outputSection.put("metrics", metrics);
		schema.put("output", outputSection);

		Map<String, Object> server = new HashMap<>();
		Map<String, Object> serverMetrics = new HashMap<>();
		serverMetrics.put("expose_fleet", Boolean.class);
		server.put("metrics", serverMetrics);
		schema.put("server", server);

		return schema;
	}

	/**
	 * Create minimal test values as fallback if full values cannot be created.
	 */
	private static Map<String, Object> createMinimalValues() {
		Map<String, Object> values = new HashMap<>();
		...
		Map<String, Object> op = new HashMap<>();
		Map<String, Object> opLimit = new HashMap<>();
		opLimit.put("count", 1000L);
		opLimit.put("rate", 100.0);
		Map<String, Object> opLimitFail = new HashMap<>();
		opLimitFail.put("count", 10L);
		opLimit.put("fail", opLimitFail);
		op.put("limit", opLimit);
		op.put("type", "write");
		load.put("op", op);
		...
		Map<String, Object> item = new HashMap<>();
		Map<String, Object> output = new HashMap<>();
		output.put("file", "test-output.csv");
		item.put("output", output);
		values.put("item", item);

		// run section values
		Map<String, Object> run = new HashMap<>();
		run.put("comment", "Test run for LoadStepClientBase");
		run.put("version", "test-version");
		values.put("run", run);
		...
		Map<String, Object> outputSection = new HashMap<>();
		Map<String, Object> metrics = new HashMap<>();
		metrics.put("quantiles", List.of(0.5, 0.95, 0.99));
		outputSection.put("metrics", metrics);
		values.put("output", outputSection);
