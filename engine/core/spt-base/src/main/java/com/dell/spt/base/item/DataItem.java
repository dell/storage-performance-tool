...
/** Random-access representation of synthetic data generated by the engine. */
public interface DataItem extends Item, SeekableByteChannel {
	...
	DataItem slice(final long from, final long size);

	/**
	 * Writes the remaining data to a socket channel.
	 *
	...
	*/
	...
	/**
	 * Write data to an asynchronous channel.
	 *
	 * <p>Warning: the handler must update the data item's position once the write completes.
	 *
	 * @param dstChan the destination channel
	 * @param dstPos the position to start writing at
	 * @param maxCount maximum number of bytes to transfer
	 * @param attach application-specific attachment passed to the handler
	 * @param handler completion handler, expected to call {@link DataItem#position(long)} with the new position
