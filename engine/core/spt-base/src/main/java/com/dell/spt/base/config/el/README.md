...
Spt is extensible with storage driver and load step plugins. It also supports scenarios scripting with any JSR-223
compatible language. Moreover, any Spt plugin may introduce its own configuration options which are being
...
be different on each value read. The new Spt version provides a general mechanism to describe the dynamic
...
The expression is being evaluated constantly in a background task thread. Requesting the expression value frequently is
