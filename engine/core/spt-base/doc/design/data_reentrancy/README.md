...
thousands of simultaneous active channels. Spt tool pre-produces some fixed amount of custom data into the memory.
...
By default spt doesn't use any external file for the ring buffer filling so it falls back to using the ring buffer
...
> Note:
> Spt supports updating the data items of the size up to 16 exabytes (2^64) because of using
> 64-bit masks.
