...
In the new major version of Spt the new distributed mode architecture is introduced. Any *distributed load step*
...
| 9  | 1 | Be able to slice the storage node addresses set among the Spt nodes involved in the particular load step run
...
Spt generates new items with the reproducible names. The new item names sequence is defined by the `item-naming-*`
