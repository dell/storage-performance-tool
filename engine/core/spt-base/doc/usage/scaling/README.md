...
```bash
java -jar spt-<VERSION>.jar --load-op-limit-rate=10000 ...
```
...
To scale up the load a custom concurrency level may be used. Spt by default limits the concurrency level with the
single active load operation at any moment of the time. This limit may be increased, for example:
```bash
java -jar spt-<VERSION>.jar --storage-driver-limit-concurrency=10000 ...
```

Also, the concurrency limit may be disabled at all (unlimited concurrency case):
```bash
java -jar spt-<VERSION>.jar --storage-driver-limit-concurrency=0 ...
```
