...
```bash
java -jar spt-<VERSION>.jar --item-output-file=items.csv ...
```
**Note**:
...
```bash
java -jar spt-<VERSION>.jar --item-output-path=/storage/path/for/the/new/items
```
...
The items output path also supports the [expression language](../../../../src/main/java/com/dell/spt/base/config/el/README.md).
