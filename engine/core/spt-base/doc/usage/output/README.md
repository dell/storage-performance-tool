...
Spt uses the [Apache Log4J2](http://logging.apache.org/log4j/2.x/) library to handle the
...
The Log4J2 configuration is bundled into the `spt.jar` resources as `log4j2.yaml`. Also the
...
`<SPT_DIR>/log/<STEP_ID>/...` where "STEP_ID" may change during runtime.
...
```bash
java -Dlog4j.configurationFile=/path/to/custom/config/log4j2.yaml -jar spt-<VERSION>.jar
```
...
java -Dlog4j.configurationFile=/path/in/container/custom-log4j2.yaml -jar /opt/spt/spt.jar "$@"
```
...
```bash
java -jar <SPT_DIR>/spt.jar --load-op-limit-count=1000 --item-output-file=items.csv
```
...
```bash
java -jar <SPT_DIR>/spt.jar --read --item-input-file=items.csv
```
...
NodeCount       | Count of the spt nodes used for the load (1 in case of the standalone mode, >1 in case of the distributed mode)
...
| `delete.complete.json` | DELETE evidence commit record: merge identity, contributors, row counts, and SHA-256 for each of the six files above. |
...
Spt controls the concurrency level by accounting the active channels at any moment of the time.
...
When Spt test step starts there's some short delay before the warmup is done and all channels
