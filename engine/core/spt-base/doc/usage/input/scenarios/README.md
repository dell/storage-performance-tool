...
Spt exposes the specific DSL to the user scenarios to run the performance tests.
...
Spt's DSL defines few basic steps and a lot of additional shortcut
steps.
...
(see `<SPT_DIR>/config/defaults.yaml` file for the reference). Subsequent calls merge the
...
The ```config``` method appends the configuration structure element to the step.
An argument should be a dictionary/map with a structure equivalent to the configuration structure
(see `<SPT_DIR>/config/defaults.yaml` file for the reference). Subsequent calls on the same
