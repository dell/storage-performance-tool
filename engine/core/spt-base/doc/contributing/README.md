...
Become one of the contributors! We thrive to build a welcoming and open community for anyone who wants to use Spt
or contribute to it.
[Here](https://github.com/thecodeteam/codedelldell.github.io/wiki/How-to-contribute-to-%7Bcode%7D-and-add-your-project)
...
[provided here](https://github.com/thecodeteam/codedelldell.github.io/wiki/Contributor-Agreement).
...
Spt uses the [semantic versioning](http://semver.org/). This means that the ***X.Y.Z*** version notation is used:
...
Tracker link: https://spt-issues.atlassian.net/projects/GOOSE
...
> scenario/use case. This approach is used due to the lack of the sufficient statistical information about the Spt
> usage.
...
| Purpose               | Story                  | Reporter: user/developer/owner | Which particular problem should be solved with Spt? The links to the related documents and literature are encouraged.
...
Spt may be distributed as a single jar. JDK 21+ is required to build.

```bash
./gradlew clean jar
```

The resulting jar file path is `./build/libs/spt-<VERSION>.jar`.
...
```bash
export SPT_IMAGE_VERSION=testing
export STORAGE_TYPE=s3
export RUN_MODE=distributed
export CONCURRENCY=medium
export ITEM_SIZE=small
./gradlew clean systemTest --tests com.dell.spt.system.CircularAppendTest
```
...
test via environment variable `SPT_IMAGE_VERSION`:
...
export CONCURRENCY=medium
export ITEM_SIZE=small
./gradlew clean systemTest --tests com.dell.spt.system.CreateNoLimitTest
```
...
./gradlew clean enduranceTest --tests com.dell.spt.endurance.ParallelPipelineAndInfiniteLoopTest
```

### 4.2.5. Robot Tests

*Note*:
> Currently the Robot tests are failing when being executed locally and passing when being executed on the [Gitlab CI](https://gitlab.com/dell-spt/spt-base/pipelines)
...
7. Update the projects depending on the Spt's API (storage drivers, etc)
