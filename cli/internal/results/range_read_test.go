...
func TestRangeReadFetchAndAbsentCompatibility(t *testing.T) {
		...
		t.Run(map[bool]string{true: "present", false: "absent"}[present], func(t *testing.T) {
			...
			handlers := map[string]http.HandlerFunc{
				...
				"/logs/" + step + "/index.json": func(w http.ResponseWriter, r *http.Request) {
					...
					if present {
						for _, name := range []string{"RangeRead", "range.read.node-000.csv"} {
			...
			for _, spec := range DefaultArtifacts {
				if spec.Suffix == "range.read.csv" {
					f.Artifacts = append(f.Artifacts, spec)
				}
			}
			if len(f.Artifacts) != 2 {
				t.Fatal("range read artifact not registered")
			}
			manifest, err := f.FetchArtifactsForSteps(context.Background(), []string{step})
			if err != nil {
				t.Fatal(err)
			}
			for _, suffix := range []string{"range.read.csv", "range.read.node-000.csv"} {
			...
			for _, file := range manifest.Steps[0].Files {
				if file.Name == step+".range.read.csv" {
					found = true
			...
			if !found {
				t.Fatal("missing range read manifest status")
			}
		})
	}
}
